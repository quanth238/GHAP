...
class Scene_subsample(Scene):
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        print("Loading trained subsampled model at {}".format(self.loaded_iter))
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse")):
            ...
            print("Found transforms_train.json file, assuming Blender data set!")
            scene_info = sceneLoadTypeCallbacks["Blender"](args.source_path, args.white_background, args.depths,
                                                           args.eval)
        else:
            assert False, "Could not recognize scene type!"
        ...
        if isinstance(self.loaded_iter, str):
            ...
            self.gaussians.load_ply(os.path.join(self.model_path,
                                                 "point_cloud",
                                                 "subsampled_" + str(self.loaded_iter),
                                                 "point_cloud.ply"),
                                                 args.train_test_exp)
            print("################")
        elif isinstance(self.loaded_iter, int):
            self.gaussians.load_ply(os.path.join(self.model_path,
                                                 "point_cloud",
                                                 "iteration_" + str(self.loaded_iter),
                                                 "point_cloud.ply"), args.train_test_exp)
    ...
    def save(self, iteration):
        point_cloud_path = os.path.join(self.model_path, "point_cloud/subsampled_{}".format(iteration))
        self.gaussians.save_ply(os.path.join(point_cloud_path, "point_cloud.ply"))
        exposure_dict = {
            image_name: self.gaussians.get_exposure_from_name(image_name).detach().cpu().numpy().tolist()
            for image_name in self.gaussians.exposure_mapping
