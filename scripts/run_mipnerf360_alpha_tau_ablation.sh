...
# Alpha-tau ablation for MipNeRF360 (RSS voxel compaction).
# Sweeps rss_alpha_tau values while keeping retention ratio fixed.

CUDA_DEVICE="${CUDA_DEVICE:-3}"
RUN_BASELINE="${RUN_BASELINE:-no}" # "yes" to train baseline checkpoints
OUT_DIR="${OUT_DIR:-./experiments}"
EXP_TITLE="${EXP_TITLE:-mipnerf360_alpha_tau_ablation}"
RUNS="${RUNS:-1}"
...
SWEEP_SUMMARY="${RESULT_DIR}/mipnerf360_alpha_tau_summary.csv"

if [[ -n "$TARGET_NUM_GAUSSIANS" && "$SAMPLING_RATIO" != "" ]]; then
  echo "[AlphaTau] TARGET_NUM_GAUSSIANS overrides SAMPLING_RATIO."
fi
...
for alpha_tau in "${ALPHA_TAUS[@]}"; do
  ...
  for run_idx in $(seq 1 "$RUNS"); do
    ...
    for scene in "${MIP_OUTDOOR_SCENES[@]}"; do
      run_scene "$scene" "images_4" "$table_out" "$run_idx" "$run_seed" "$alpha_tau"
    done
    for scene in "${MIP_INDOOR_SCENES[@]}"; do
      run_scene "$scene" "images_2" "$table_out" "$run_idx" "$run_seed" "$alpha_tau"
    done
